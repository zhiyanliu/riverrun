...
This is the code repository of an agent to transform and synchronize video streaming data and video structuring data of each frame running on [edge device](https://en.wikipedia.org/wiki/Edge_computing).
...
- Function `VideoProcessor`
    ...
    |SYNC_PACKET_POLICY_SEND_METADATA_FRAME_ONCE|If to send multiple same video metadata frames with every RTP fragmentation units of the video frame in the synchronization packet. Option `No` means only send one video metadata frame with the first RTP fragmentation unit of the video frame.|String (Boolean)               |`Yes`, `No`           |`No`         |
