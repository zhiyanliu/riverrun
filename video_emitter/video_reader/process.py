...
def _server_process_watchdog(server_creator, server_serving_event, server_released_event,
                             server_creator_args, server_serve_kwargs):
    global _server_process_server

    _server_process_server = server_creator(*server_creator_args)

    print("%s is serving, pid = %d" % (_server_process_server.name(), os.getpid()))
    server_serving_event.set()
    _server_process_server.serve(**server_serve_kwargs)
    print("%s exits" % _server_process_server.name())
    _server_process_server.release()
    print("%s released" % _server_process_server.name())
...
class ServerProcess:
    ...
    def start(self, **kwargs):
        ...
        self._pid = multiprocessing.Process(name=self._name, target=_server_process_watchdog,
                                            args=(self._server_creator, server_serving_event, self._released_event,
